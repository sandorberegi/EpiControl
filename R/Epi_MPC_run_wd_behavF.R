...
Epi_MPC_run_wd_behavF <- function(episimdata, epi_par, noise_par, actions, behav, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 1, N = 1e6) {
  ...
  cost_cum <-behav["cost_cum"]
  cost_cum_shape <- behav["cost_cum_shape"]
  emo_lam <-behav["emo_lam"]
  emo_R <-behav["emo_R"]
  emo_fatigue_coeff <-behav["emo_fatigue_coeff"]
  ...
  for (ii in (start_day+1):ndays) {
    ...
    u1cum <- cost_cum * Rcoeff_cost * tanh(-1/exp(episimdata[ii-1, "Reserves"]/cost_cum_shape))
    u0emo <- - (emo_fatigue_coeff ** episim_data[ii, "LD_count"]) *  (emo_lam * episimdata[ii, 'Rest'] * episimdata[ii-1, "Lambda_C"] + emo_R * episimdata[ii, 'Rest'])
    ...
    u0 <- ind_impact*u0S + (1-ind_impact)*u0I + u0emo
    u1 <- ind_impact*u1S + (1-ind_impact)*u1I + u1cum

    episimdata[ii, "u0_cost"] <- u0
    episimdata[ii, "u1_cost"] <- u1

    episimdata[ii, "Cost_cum"] <- u1cum
    episimdata[ii, "Cost_emo"] <- u0emo
    ...
    episimdata[ii, "Reserves"] <- episimdata[ii-1, "Reserves"] - episimdata[ii, "p"] * Rcoeff_cost
