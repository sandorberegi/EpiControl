...
epicontrol <- function(episim_data_ens, settings) {
  sim_function <- episettings$sim_function
  projection_function <- episettings$projection_function
  ...
  if (parallel) {
    ...
    for (jj in 1:sim_ens) {
      episim_data_ens[[jj]] <- sim_function(episim_data_ens[[jj]], epi_par, noise_par, actions, pred_days = sim_settings$pred_days, n_ens = sim_settings$n_ens, start_day = sim_settings$start_day, ndays = sim_settings$ndays, R_est_wind = sim_settings$R_est_wind, pathogen = sim_settings$pathogen, susceptibles = sim_settings$susceptibles, delay = sim_settings$delay, ur = sim_settings$ur, r_dir = sim_settings$r_dir, N = sim_settings$N)
