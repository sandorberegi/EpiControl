...
Epi_MPC_run_wd_behav2 <- function(episimdata, epi_par, noise_par, actions, behav, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 1, N = 1e6) {
  ...
  for (ii in (start_day+1):ndays) {
    ...
    episimdata[ii, 'R0est'] <- episimdata[ii, 'Rest'] / (1-eta_tmp*episimdata[ii-1, "p"])
    ...
    u0 <- -episimdata[ii-1, "p"]*Ract*episimdata[ii-1, "Lambda_C"]-(1-episimdata[ii-1, "p"])*(Ract/Rcoeff)*episimdata[ii-1, "Lambda_C"] - cost_diff*(episimdata[ii-1, "p"])^2
    u1 <- -episimdata[ii-1, "p"]*Rcoeff*Ract*episimdata[ii-1, "Lambda_C"] - (1-episimdata[ii-1, "p"])*Ract*episimdata[ii-1, "Lambda_C"] - cost_q*Rcoeff_cost - cost_diff*(episimdata[ii-1, "p"]-1)^2
