...
Epi_MPC_run_wd_behav_3ways <- function(episimdata, epi_par, noise_par, actions, behav, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 1, N = 1e6) {
  ...
  for (ii in (start_day+1):ndays) {
    ...
    uN <- -(Ract/Rcoeff)*episimdata[ii-1, "Lambda_C"] #- cost_diff*(episimdata[ii-1, "pp"])^2
    uP <- -Ract*episimdata[ii-1, "Lambda_C"] - cost_q*Rcoeff_cost #- cost_diff*(episimdata[ii-1, "pp"]-1)^2
    uO <- -(Ract/Rcoeff)*(1-eta2)*episimdata[ii-1, "Lambda_C"] - cost_O*cost_q #- cost_diff*(episimdata[ii-1, "pp"]-eta2/(1-Rcoeff))^2
