...
Epi_MPC_run_wd_behav_info <- function(episimdata, epi_par, noise_par, actions, behav, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 1, N = 1e6) {
  ...
  for (ii in (start_day+1):ndays) {
    ...
    if (ii %% rf == 0L) {
      ...
      for (jj in 1:number_of_actions){
        ...
        for (kk in 1:n_ens){
          Reward_ens[kk] <- Epi_pred_wd_behav(episimdata, epi_par, noise_par, actions, behav, pathogen, pred_days, r_dir, ii, jj, N)
        }
