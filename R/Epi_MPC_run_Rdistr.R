...
Epi_MPC_run_Rdistr <- function(episimdata, epi_par, noise_par, actions, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, R_uncert = 1, c_uncert = 1, N = 1e6) {

  if (R_uncert == 1) {
    R0shape <- epi_par[pathogen, "R0_shape"]
    R0rate <- epi_par[pathogen, "R0_rate"]

    R0 <- rgamma(1, shape = R0shape, rate = R0rate)
  ...
  for (ii in (start_day+1):ndays) {
    ...
    if (ii %% rf == 0L) {
      ...
      if ((c_uncert == 1) && (episimdata[ii, 'policy'] != 1) && (episimdata[ii, 'policy'] != episimdata[ii-1, 'policy'])){

        R_beta_a <- actions[episimdata[ii, 'policy'], 'R_beta_a']
        R_beta_b <- (1 - actions[episimdata[ii, 'policy'], 'R_coeff']) / actions[episimdata[ii, 'policy'], 'R_coeff'] * R_beta_a

        Rcoeff <- rbeta(1, shape1 = R_beta_a, shape2 = R_beta_b)
      ...
      Rcoeff <- episimdata[ii-1, 'R_coeff']
      episimdata[ii, 'R_coeff'] <- Rcoeff
    }
