...
Epi_MPC_run_wd_thr2 <- function(episimdata, epi_par, noise_par, actions, pred_days, n_ens = 100, start_day = 1, ndays = nrow(episimdata), R_est_wind = 5, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 0, N = 1e6) {
  ...
  for (ii in (start_day+1):ndays) {
    ...
    if ((ii %% rf == 0L)) {
      if (episimdata[ii-1, 'policy'] == 1L) {
        if ((episimdata[ii-1, 'Deaths'] > LD_on) & ((ii - last_changed) > 7)){
        ...
        if ((episimdata[ii-1, 'Deaths'] < LD_off) & ((ii - last_changed) > 7)){
