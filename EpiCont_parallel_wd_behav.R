...
results <- pblapply(1:sim_ens, function(jj) {
  episim_data_ens[[jj]] <- Epi_MPC_run_wd_behav(episim_data_ens[[jj]], Epi_pars, Noise_pars, Action_space, behave_pars, pred_days = pred_days, start_day = 1, n_ens = n_ens, ndays = nrow(episim_data), R_est_wind = R_est_wind, pathogen = 1, susceptibles = 0, delay = 1, ur = 0, r_dir = 2, N = N)
}, cl=cl)
