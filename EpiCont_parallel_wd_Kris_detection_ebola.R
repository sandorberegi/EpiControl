...
results <- pblapply(1:sim_ens, function(jj) {
  episim_data_ens[[jj]] <- Epi_MPC_run_wd_K(episim_data_ens[[jj]], Epi_pars, Noise_pars, Action_space, pred_days = pred_days, start_day = 1, n_ens = n_ens, ndays = nrow(episim_data), R_est_wind = R_est_wind, pathogen = 2, susceptibles = 0, delay = 1, ur = 1, r_dir = 1, N = N)
}, cl=cl)
...
saveRDS(episim_data_ens, file = "episim_data_KP_paper_with_noise.rds")
...
save(episim_data_ens, file = "episim_data_ens_with_noise_ebola.RData")
