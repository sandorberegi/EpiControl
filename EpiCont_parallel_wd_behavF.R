...
column_names <- c("days", "sim_id", "I", "Lambda", "C", "Lambda_C", "S", "Deaths", "Re", "Rew", "Rest", "R0est", "policy", "R_coeff", "p", "Reserves", "LD_count", "SD_count", "u0_cost", "u1_cost", "Cost_cum", "Cost_emo")
...
episim_data[1,] <- c(1, 1, I0, I0, Noise_pars['ur_mean']*I0, Noise_pars['ur_mean']*I0, N-I0, 0, Epi_pars[1,'R0'], Epi_pars[1,'R0'], 1, 1, 1, 1, 1, 15, 0, 0, 0, 0, 0, 0)
...
results <- pblapply(1:sim_ens, function(jj) {
  episim_data_ens[[jj]] <- Epi_MPC_run_wd_behavF(episim_data_ens[[jj]], Epi_pars, Noise_pars, Action_space, behave_pars, pred_days = pred_days, start_day = 1, n_ens = n_ens, ndays = nrow(episim_data), R_est_wind = R_est_wind, pathogen = 1, susceptibles = 0, delay = 1, ur = 1, r_dir = 2, N = N)
}, cl=cl)
...
#for (jj in 1:sim_ens) {
#  episim_data_ens[[jj]] <- Epi_MPC_run_wd_behav4(episim_data_ens[[jj]], Epi_pars, Noise_pars, Action_space, behave_pars, pred_days = pred_days, start_day = 1, n_ens = n_ens, ndays = nrow(episim_data), R_est_wind = R_est_wind, pathogen = 1, susceptibles = 1, delay = 0, ur = 0, r_dir = 1, N = N)
...
ggplot(combined_data %>% filter(sim_id == 1)) +
  ...
  geom_line(aes(x = days, y = 5000*p, color = factor(policy, labels = policy_labels), group = 1), alpha = 1.0, size=0.25) +
...
ggplot() +
  geom_line(data = subset(combined_data, sim_id != 1), aes(x = days, y = Reserves, color = as.factor(sim_id)), alpha = 0.3) +
  geom_line(data = subset(combined_data, sim_id == 1), aes(x = days, y = Reserves), color = "red", alpha = 1.0) +
  labs(x = "Days", y = "Reserves") +
  scale_color_manual(values = cls) +
  guides(color = FALSE)


ggplot() +
  geom_line(data = subset(combined_data, sim_id != 1), aes(x = days, y = -u0_cost, color = as.factor(sim_id)), alpha = 0.3) +
  geom_line(data = subset(combined_data, sim_id == 1), aes(x = days, y = -u0_cost), color = "red", alpha = 1.0) +
  geom_line(data = subset(combined_data, sim_id != 1), aes(x = days, y = -u1_cost, color = as.factor(sim_id)), alpha = 0.3) +
  geom_line(data = subset(combined_data, sim_id == 1), aes(x = days, y = -u1_cost), color = "blue", alpha = 1.0) +
  labs(x = "Days", y = "costs") +
  scale_color_manual(values = cls) +
  guides(color = FALSE)

ggplot() +
  geom_step(data = subset(combined_data, sim_id == 1), aes(x = days, y = -u0_cost), color = "red", alpha = 1.0) +
  geom_step(data = subset(combined_data, sim_id == 1), aes(x = days, y = -Cost_cum), color = "darkblue", alpha = 1.0) +
  geom_step(data = subset(combined_data, sim_id == 1), aes(x = days, y = -u1_cost), color = "blue", alpha = 1.0) +
  geom_step(data = subset(combined_data, sim_id == 1), aes(x = days, y = -Cost_emo), color = "purple", alpha = 1.0) +
  labs(x = "Days", y = "costs") +
  scale_color_manual(values = cls) +
  guides(color = FALSE)
