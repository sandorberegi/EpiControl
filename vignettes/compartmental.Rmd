...
### Simulation/model projecttion parameters and cost function
...
When we plot the results we observe that the controller aims keeping daily incidence near the target level. As immunity in the population builds up, this can be achieved by periods of "social distancing" and no intervention whilst the most stringent measure, "lockdown" is used only once at the beginning when population immunity is low (below 20%). Once herd immunity is reached no further intervention is applied. At this point above 20% of the population has infection acquired immunity (recovered) and another 45% is protected by the vaccine.
...
We can see that the accelerated vaccination programme leads to a shorter outbreak and roughly a 50% reduction in the total number of infections. With the higher vaccination rate, herd immunity is reached after 420 days rather than 680, and by that point only around 10% of the population has been infected.
