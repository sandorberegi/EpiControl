---
title: "Ebola example - using EpiEstim for R estimation"
...
cat("## Overview\n\n",
    "This vignette demonstrates model predictive optimal control of Ebola Virus Disease 
    cases in West Africa. We initialise the simulation from real data and roughly re-create the dynamics of the outbreak between 2013–2016. We will also demonstrate integration of EpiControl with EpiEstim. \n\n")
```
...
We will first load the dataset containing cases of West African Ebola.
Garske, T. et al. Heterogeneities in the case fatality ratio in the West African Ebola outbreak 2013–2016. Philos. Trans. R. Soc. Lond. B Biol. Sci. 372, 20160308 2017. [link](https://royalsocietypublishing.org/doi/10.1098/rstb.2016.0308)
...
colnames(date_counts) <- c("date_reported", "count") #actual case-count
colnames(date_counts_i) <- c("date_reported", "count") #inferred cases
...
We set the simulation parameters, targets, R estimation window etc. below.
We do not account for vaccination or new variants, therefore we set v_max_rate = 0 and delta_multiplier = 1.0.
Our target is to keep cases at 50 cases/day.
...
sim_settings <- list(
  ...
  pred_days = 21L,
  r_trans_steep = 1.5,  # Transition steepness
  r_trans_len = r_trans_len,  # Number of days for the transition between policies
